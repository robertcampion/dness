...
impl DnsLookupProvider for HeProvider<'_> {
    async fn update_domain(&self, record: &str, wan: IpAddr) -> Result<(), DnessError> {
        ...
        let response = request
            .send()
            .await
            .map_err(|e| DnessError::send_http(&self.get_url, "he update", e))?
            .error_for_status()
            .map_err(|e| DnessError::bad_response(&self.get_url, "he update", e))?
            .text()
            .await
            .map_err(|e| DnessError::deserialize(&self.get_url, "he update", e))?;
