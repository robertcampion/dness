...
pub async fn ipify_resolve_ip(client: &reqwest::Client, ip_type: IpType) -> Result<IpAddr> {
    ...
    let ip_text = client
        ...
        .send()
        .await
        .context(HttpError::send(ipify_url, "ipify get ip"))?
        .error_for_status()
        .context(HttpError::bad_response(ipify_url, "ipify get ip"))?
        .text()
        .await
        .context(HttpError::deserialize(ipify_url, "ipify get ip"))?;
